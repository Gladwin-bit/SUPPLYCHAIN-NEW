...
// Customer scans QR → browser opens /product/:id → views journey → enters scratch code
const buildConsumerPayload = (productId) =>
...
export default function BulkRegister() {
        ...
        try {
            ...
            for (const p of registered.products) {
                const png = await QRCode.toDataURL(
                    buildConsumerPayload(p.productId),
            ...
            const csvRows   = registered.products.map(p => {
                const qr = buildConsumerPayload(p.productId);
            ...
            const formattedBatchId = registered.formatted?.batchId?.replace(/[^a-zA-Z0-9]/g, '') || registered.batchId;
            saveAs(blob, `batch_${formattedBatchId}_export.zip`);
    ...
    const downloadWaybill = async () => {
        try {
            ...
            link.href = url;
            const formattedBatchId = registered.formatted?.batchId?.replace(/[^a-zA-Z0-9]/g, '') || registered.batchId;
            link.download = `batch_${formattedBatchId}_waybill.png`;
            ...
            try {
                ...
                const formattedBatchId = registered.formatted?.batchId?.replace(/[^a-zA-Z0-9]/g, '') || registered.batchId;
                link.download = `batch_${formattedBatchId}_waybill_fallback.png`;
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                                            ...
                                            {registered.products.map(p => (
                                                <div key={p.productId} className="br-product-item glass">
                                                    <div className="br-product-top">
                                                        ...
                                                        <QRCodeSVG
                                                            value={buildConsumerPayload(p.productId)}
