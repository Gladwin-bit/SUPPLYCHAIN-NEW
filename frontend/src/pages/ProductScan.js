...
export default function ProductScan() {
    const { id } = useParams();
    // Parse the ?batch= URL parameter if present
                            ...
                            {product.history.map((entry, i) => {
                                ...
                                return (
                                    <div key={i} className="ps-tl-node">
                                        ...
                                        {/* Dot */}
                                        <div className="ps-tl-dot" style={{ 
                                            background: entry.state === "In Transit (Batch)" ? "#fbbf24" : (isLast ? meta.color : "#4b5563"),
                                            boxShadow: isLast ? `0 0 0 4px ${meta.color}30` : "none",
                                            fontSize: entry.state === "In Transit (Batch)" ? "0.85rem" : "1rem"
                                        }}>
                                            {entry.state === "In Transit (Batch)" ? "📦" : meta.icon}
                                        </div>
